...
Built against a genuinely `no_std` `marshal-rs-capi` (`--no-default-features`), registering `malloc`/`free`-backed hooks via `mrs_set_allocator` before making any other allocating call.
