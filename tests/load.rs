...
use marshal_rs::{load, load_binary};
use serde_json::json;
...
#[test]
fn null() {
    assert_eq!(load(b"\x04\x080", None).unwrap(), json!(null));
}

#[test]
fn boolean() {
    assert_eq!(load(b"\x04\x08T", None).unwrap(), json!(true));
    assert_eq!(load(b"\x04\x08F", None).unwrap(), json!(false));
}

#[test]
fn fixnum_positive() {
    assert_eq!(load(b"\x04\x08i\0", None).unwrap(), json!(0));
    assert_eq!(load(b"\x04\x08i\x0A", None).unwrap(), json!(5));
    assert_eq!(load(b"\x04\x08i\x02\x2C\x01", None).unwrap(), json!(300));
    assert_eq!(
...
#[test]
fn fixnum_negative() {
    assert_eq!(load(b"\x04\x08i\xF6", None).unwrap(), json!(-5));
    assert_eq!(load(b"\x04\x08i\xFE\xD4\xFE", None).unwrap(), json!(-300));
    assert_eq!(
...
fn links() {
    assert_eq!(
        ...
        .unwrap(),
        json!([
            ["__float__0.1", "__float__0.1", "__float__0.1"],
            ["__float__0.2", "__float__0.2", "__float__0.2"],
            ["__float__0.3", "__float__0.3", "__float__0.3"]
        ])
    );
}
...
fn array() {
    assert_eq!(
        ...
        .unwrap(),
        json!([1, "two", "__float__3", [4], {"__integer__5": 6}])
    );
}
